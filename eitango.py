...
# ===================== 出題形式・問題数選択画面 =====================
    ...
    for i, m in enumerate(modes):
        label = m
        if st.session_state.mode == m: label += " (選択中)"
        if mode_cols[i].button(label, key=f"mode_{i}"):
    ...
    for i, c in enumerate(counts):
        label = str(c)
        if st.session_state.question_count == c: label += " (選択中)"
        if count_cols[i].button(label, key=f"count_{i}"):
...
# ===================== クイズ画面 =====================
    ...
    with st.form(f"quiz_form_{q['id']}"):
        answer = st.text_input("英語を入力してください", value=st.session_state.user_answers[n], key=f"text_{q['id']}")
        my = st.checkbox("⭐ My単語に追加", value=st.session_state.user_my_flags[n], key=f"my_{q['id']}")
    ...
    if spc_cols[1].button("⌫", key=f"del_{q['id']}"):
        ...
        st.experimental_rerun()
    if spc_cols[2].button("enter", key=f"enter_{q['id']}"):
        st.session_state.judged = "correct" if st.session_state.user_answers[n].lower() == q["en"].lower() else "wrong"
        st.rerun()
    ...
    if st.session_state.judged is not None:
        ...
        if st.button("次へ", key=f"next_{q['id']}"):
...
# ===================== セット終了画面 =====================
    ...
    for i, (q, answer, my_flag) in enumerate(zip(questions, st.session_state.user_answers, st.session_state.user_my_flags)):
        ...
        with col1: st.markdown("✅" if answer.lower()==q["en"].lower() else "❌")
